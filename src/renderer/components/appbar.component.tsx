import { AppBar, Box, Drawer, IconButton, List, ListItem, ListItemIcon, ListItemText, Toolbar, Tooltip, Typography } from "@mui/material";
import {
	Menu as MenuIcon,
...
export const MyAppBar: FC = () => {
	...
	const navs: { [key: string]: [string, string, JSX.Element] } = {
		...
		'/reports': [t('titles.reports'), t('descriptions.reports'), <ArticleIcon />],
		...
		<>
			<AppBar position='static' enableColorOnDark>
				<Toolbar>
					...
					<Typography variant='h6' sx={{ flexGrow: 1 }}>{navs[location.pathname][0] ?? ''}</Typography>
					<Tooltip title={t('changeLanguage') as string}>
						<IconButton color='inherit' size='large' onClick={() => i18n.changeLanguage(i18n.language === 'en' ? 'ar' : 'en')}>
							<TranslateIcon />
						</IconButton>
					</Tooltip>
					<Tooltip title={t('changeMode') as string}>
						<IconButton color='inherit' size='large' onClick={() => setMode(mode === 'light' ? 'dark' : 'light')}>
			...
			<Drawer anchor='left' open={showDrawer} onClose={() => setShowDrawer(false)}>
				<Box role='presentation' sx={{ width: 350 }} onClick={() => setShowDrawer(false)}>
					<List>
					{Object.keys(navs).map(key => (
						<ListItem key={key} button selected={key === location.pathname} onClick={() => navigate(key)}>
